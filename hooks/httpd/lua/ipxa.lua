...
function ip_info_check(r)
    ...
    if cached then
        ...
        if code ~= 200 then
            return utils.respond(r, apache2.DECLINED, "[skip/error]: " .. ip .. " code=" .. code)
        end

        country_code = utils.get_header(headers, "x-country-code")
        risk_score = tonumber(utils.get_header(headers, "x-risk-score") or -1) or -1
        trusted = (string.lower(utils.get_header(headers, "x-trusted")) == "true")

        if country_code and risk_score ~= -1 then
            cache.set(ip, {country_code = country_code, risk_score = risk_score, trusted = trusted})
        end
    end
    ...
    if country_code and BLOCKED_COUNTRIES[country_code] then
        return utils.respond(r, 403, "[block/geo-ip]: " .. ip .. " country_code=" .. country_code,
            {
    ...
    if risk_score > 0 then
        return utils.respond(r, 403, "[block/risk-score]: " .. ip .. " risk_score=" .. risk_score,
            {
                ["x-country-code"] = country_code,
                ["x-risk-score"] = risk_score
            }
